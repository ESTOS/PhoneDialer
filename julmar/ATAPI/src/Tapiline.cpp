...
void CTapiLine::GatherAddressInformation()
{
	if (GetLineCaps() != NULL)
	{
		...
		if (m_arrAddress.GetCount() > (int)GetAddressCount())
		{
			...
			{
				int idx = (int)m_arrAddress.GetCount() - 1;
				CTapiAddress* pAddr = (CTapiAddress*)m_arrAddress.GetAt(idx);
				delete pAddr;
				m_arrAddress.RemoveAt(idx);
			}
		}
...
{
	const LPLINEDEVCAPS pLineCaps = GetLineCaps();
	if (!pLineCaps)
		return NULL;

	if (pLineCaps->dwDevSpecificOffset && pLineCaps->dwDevSpecificSize >= sizeof(ECSTALINEDEVCAPS4))
	{
		ECSTALINEDEVCAPS4* pECSTADevCaps = (ECSTALINEDEVCAPS4*)((BYTE*)pLineCaps + pLineCaps->dwDevSpecificOffset);
		return pECSTADevCaps;
	}
	return NULL;
}
...
{
	return DevSpecificFeature(ECSTA150_FEATURE_AGENTSTATEREQUEST, (void*)pAgentStateRequest, sizeof(ECSTA150AGENTSTATEREQUEST21));
}

LONG CTapiLine::DevSpecific(DWORD dwAddressID, HCALL hCall, LPVOID lpParams, DWORD dwSize)
{
...
LONG CTapiLine::GetECSTAInvalidPasswordState()
{
	LONG lInvalidPasswordState = 0;
	ETSPVarStruct<VARSTRING> invalidPasswordState;
