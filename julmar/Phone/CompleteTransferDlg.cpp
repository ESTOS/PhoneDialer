...
BEGIN_MESSAGE_MAP(CCompleteTransferDlg, CDialog)
	...
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_CMDCLOSE, OnCloseWindow)
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_CONFERENCE, &CCompleteTransferDlg::OnBnClickedConference)
END_MESSAGE_MAP()
...
BOOL CCompleteTransferDlg::OnInitDialog() 
{
	...
	if (m_fConference)
	{
		SetWindowText(_T("Complete the Conference"));
		GetDlgItem(IDOK)->SetWindowText(_T("Conference"));
...
void CCompleteTransferDlg::UpdateTransferButton()
{
	...
	{
		if (pConsStatus->dwCallFeatures & LINECALLFEATURE_ADDTOCONF)
			bEnableButton = true;
	}
...
void CCompleteTransferDlg::OnCloseWindow()
{
	EndDialog(IDC_CMDCLOSE);
}

void CCompleteTransferDlg::OnBnClickedConference()
{
	UpdateTransferButton();
}
