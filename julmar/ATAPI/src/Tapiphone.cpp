...
void CTapiPhone::Init(CTapiConnection* pConn, DWORD dwDeviceID)
{
	...
	if (phoneNegotiateAPIVersion(pConn->GetPhoneAppHandle(), dwDeviceID, TAPIVER_13, TAPIVER_21, &m_dwAPIVersion, &phoneExtID) == 0)
	{
...
CString CTapiPhone::GetProviderInfo() const
{
	...
	if (m_lpPhoneCaps)
	{
		if (m_lpPhoneCaps->dwProviderInfoSize && m_lpPhoneCaps->dwProviderInfoOffset)
		//            m_lpPhoneCaps->dwStringFormat == STRINGFORMAT_ASCII)
		{
			LPCTSTR lpszProvider = (LPCTSTR)(((LPBYTE)m_lpPhoneCaps) + m_lpPhoneCaps->dwProviderInfoOffset);
...
CString CTapiPhone::GetPhoneName() const
{
	...
	if (lpCaps)
		...
		{
			LPCTSTR lpszName = (LPCTSTR)(((LPBYTE)lpCaps) + lpCaps->dwPhoneNameOffset);
			strName = lpszName;
		}
		else
...
const LPPHONECAPS CTapiPhone::GetPhoneCaps(DWORD dwAPIVersion, DWORD dwExtVersion, BOOL fForceRealloc)
{
	...
	while (TRUE)
	{
		...
		if (phoneGetDevCaps(m_pConn->GetPhoneAppHandle(), GetDeviceID(), dwAPIVersion, dwExtVersion, m_lpPhoneCaps) != 0)
			return NULL;
...
LONG CTapiPhone::Open(DWORD dwPrivilege, DWORD dwAPIVersion, DWORD dwExtVersion)
{
	...
	LONG lResult = phoneOpen(m_pConn->GetPhoneAppHandle(), GetDeviceID(), &m_hPhone, dwAPIVersion, dwExtVersion, (DWORD_PTR)this, dwPrivilege);
	if (lResult != 0)
		m_hPhone = NULL;
	return lResult;
...
// changing.
//
void CTapiPhone::PhoneCallback(DWORD hDevice, DWORD dwMsg, DWORD dwParam1, DWORD dwParam2, DWORD dwParam3)
{
	switch (dwMsg)
	{
		case PHONE_BUTTON:
...
//
void CTapiPhone::OnDevSpecific(DWORD /*dwHandle*/, DWORD /*dwParam1*/, DWORD /*dwParam2*/, DWORD /*dwParam3*/)
{
	/* Do nothing */
...
{
	return phoneConfigDialog(m_dwDeviceID, (pwndOwner) ? pwndOwner->GetSafeHwnd() : NULL, lpszDeviceClass);
...
DWORD CTapiPhone::GetProviderID() const
{
	...
	if (((CTapiPhone*)this)->GetID(lpVS, _T("tapi/providerid")) == 0 && lpVS->dwStringSize == sizeof(DWORD) && lpVS->dwStringFormat == STRINGFORMAT_BINARY)
...
LONG CTapiPhone::GetButtonInfo(int iButtonID, LPBUTTONINFO lpButton)
{
	...
	if (lpInfo->dwButtonTextOffset > 0)
		lpButton->strButtonText = (LPCTSTR)((LPBYTE)lpInfo + lpInfo->dwButtonTextOffset);
	else
