...
BOOL CConfListDlg::OnInitDialog() 
{
	...
	for (POSITION pos = lstCalls.GetHeadPosition(); pos != NULL;)
	{
		...
		{
			strBuff.Format(_T("Owner x%lx - %s"), pCall, (const wchar_t*)pCall->GetCallStateString());
			m_lbList.InsertString(0, strBuff);
		}
		else
		{
			strBuff.Format(_T("   Member x%lx - %s"), pCall, (const wchar_t*)pCall->GetCallStateString());
