...
DWORD CTapiCall::GetECSTACallFeatures()
{
	DWORD dwECSTAFeatures = 0;
	if (!m_lpCallStatus)
		return dwECSTAFeatures;

	if (!m_lpCallStatus->dwDevSpecificOffset || !m_lpCallStatus->dwDevSpecificSize)
		return dwECSTAFeatures;

	if (m_lpCallStatus->dwDevSpecificSize >= sizeof(ECSTADEVSPECIFICELEMENTLIST))
	{
		ECSTADEVSPECIFICELEMENTLIST* pList = (ECSTADEVSPECIFICELEMENTLIST*)(((LPBYTE)m_lpCallStatus) + m_lpCallStatus->dwDevSpecificOffset);
